...
f_leu_plasma = [x[0] for x in pd.read_csv('../../Data/bolus_800_plasma_new.csv', header=None).values]
f_leu_csf = [x[0] for x in pd.read_csv('../../Data/bolus_800_csf_new.csv', header=None).values]
...
plasma_vals = [(x*0.683992759)*(5.7e-3 + 0.952e-3) for x in plasma_df.iloc[:, 1]]
plasma_vals_auc = [(x*0.8799657293261557)*(5.7e-3 + 0.952e-3) for x in plasma_df.iloc[:, 1]]
...
#plt.plot(plasma_times, plasma_vals_auc, marker='x', label='Observed')
plt.errorbar(x_error[:14], y_error, yerr=errors, label='Observed', fmt='o', capsize=5)
plt.xlabel("Time, h")
plt.ylabel("Labeled plasma Amyloid, nM")
plt.legend()
plt.tight_layout()
plt.show()
#fig1.savefig('output/no_ab_new_prefit/SILK_Plasma_SILK.png', dpi=300)
...
plt.legend()
plt.show()
#fig2.savefig('output/no_ab_new_prefit/SILK_Plasma_SILK_leucine.png', dpi=300)
